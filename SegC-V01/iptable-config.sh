...
sudo iptables -A OUTPUT -p icmp -m icmp --icmp-type 8 -s 10.101.148.0/23 -m limit --limit 2/second -j ACCEPT
...
#sudo /sbin/iptables -A INPUT -p tcp -m state --state NEW -m tcp --dport 22 -m iprange -s 10.101.148.0/23 -j ACCEPT
